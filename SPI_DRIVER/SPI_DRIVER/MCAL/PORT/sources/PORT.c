...
PORT_enu_return_status_t  PORT_enu_SetPinLogic(PORT_enu_PinId_t arg_enu_PinId,PORT_enu_PinLogic_t arg_enu_PinLogic)
{
	...
	else if ( arg_enu_PinLogic >= PIN_LOGIC_ERROR)
	{
		loc_enu_return_status = STATUS_PORT_PINMODE_CHOICE_ERROR;
	}
	else
	{
		...
		switch(loc_u8_portNumber)
		{
			case PORTA_VALUE :
			{
				if(arg_enu_PinLogic == PIN_LOGIC_LOW)
				{
					CLR_BIT(PORTA,loc_u8_pinNumber);
				}
				else if(arg_enu_PinLogic == PIN_LOGIC_HIGH)
				{
					SET_BIT(PORTA,loc_u8_pinNumber);
				}
