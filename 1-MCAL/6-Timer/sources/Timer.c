...
void TIMER_2_voidInit(Timer2_cfg * prt_TIMER2_CFG){
    ...
    if (prt_TIMER2_CFG->OC0_MODE == OC0_MODE_NORMAL_PORT){
        CLR_BIT(TCCR2, 5); 
        CLR_BIT(TCCR2, 4);
    }else if (prt_TIMER2_CFG->OC0_MODE == OC0_MODE_TOGGLE_ON_CTC){
        CLR_BIT(TCCR2, 5); 
        SET_BIT(TCCR2, 4);
    }else if (prt_TIMER2_CFG->OC0_MODE == OC0_MODE_CLEAR_ON_CTC_NON_INV){
        SET_BIT(TCCR2, 5); 
        CLR_BIT(TCCR2, 4);
    }else if (prt_TIMER2_CFG->OC0_MODE == OC0_MODE_SET_ON_CTC_INV){
        SET_BIT(TCCR2, 5); 
        SET_BIT(TCCR2, 4);
    }
