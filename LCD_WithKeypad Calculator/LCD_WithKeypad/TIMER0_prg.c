...
void TIMER0_voidInit(void)
{
	...
	SET_BIT(TIFR,TOV0);
	CLR_BIT(TIFR,OCF0);
	#elif TIMER0_interrupt == COMPARE_MATCH_INT
	CLR_BIT(TIFR,TOV0);
	SET_BIT(TIFR,OCF0);
	#elif TIMER0_interrupt == OVERFLOW_AND_COMPARE_MATCH_INT
	SET_BIT(TIFR,TOV0);
	SET_BIT(TIFR,OCF0);	
	#elif TIMER0_interrupt == DISABLE_INT
	CLR_BIT(TIFR,TOV0);
	CLR_BIT(TIFR,OCF0);	
	#endif
}
...
{
	f32 Local_f32TimFreq   = 16 / Copy_u32Prescaller;
